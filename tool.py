...
def parse_api_result_ver_2(result):
    to_return = []
    for idx, g in enumerate(result['choices']):
...
def safe_execute(code_string: str, key=None):
    def execute(x, key=None):
        try:
            exec(x)
            locals_ = locals()
            if key is None:
                ...
                return locals_.get(key, None)
        except Exception:
            return None
    try:
        ans = func_timeout.func_timeout(5, execute, args=(code_string,key))
