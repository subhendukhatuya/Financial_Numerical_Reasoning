...
def load_data(args):
    # problems = json.load(open(os.path.join(args.data_root, f'finqa_final_correct_program_train_samples_formatted_for_promptpg.json')))
    problems = json.load(open(os.path.join(args.data_root, f'promptpg_style_train_samples_50.json')))
...
def get_batch_reward_loss(scores, cand_pids, pid_batch, option_batch, unit_batch, label_batch, args):
    ...
    for i in range(len(scores)):
        ...
        answer = problems[pid_batch[i]]["answer"];
        print(problems[pid_batch[i]]);
        print("\n\nanswer: "+str(answer))
        # print(5/0);
        ...
        codes, outputs = parse_api_result(output)
        print("prompt:\t"+ prompt);
        print("\noutput:"+ outputs);
        ...
        if prediction is None:
            _reward = -1
        elif finqa_equal(prediction, answer, False):
            _reward = 1
        else:
            _reward = -1
...
def policy_gradient_train(policy_model, problems, train_pids, cand_pids, cand_examples, args, saving):
    # REINFORCE
    ...
    for epoch in range(args.epochs):
        ...
        if epoch == best_reward_epoch:
            ckpt_file = os.path.join(args.ckpt_path, f"ckpt_best_reward_{saving}.pt")
            torch.save(policy_model.linear.state_dict(), ckpt_file)
            logger.write(f"saved the best reward ckpt to {ckpt_file}")

        if epoch == best_loss_epoch:
            ckpt_file = os.path.join(args.ckpt_path, f"ckpt_best_loss_{saving}.pt")
            torch.save(policy_model.linear.state_dict(), ckpt_file)
    ...
    # save in the end
    ckpt_file = os.path.join(args.ckpt_path, f"ckpt_final_{saving}.pt")
    # ckpt_file = os.path.join(args.ckpt_path, "ckpt_final_2.pt")
    torch.save(policy_model.linear.state_dict(), ckpt_file)
...
if __name__ == '__main__':
    ...
    for i in range(epoch_s):
        ...
        ## TRAINING
        logger = utils.Logger(os.path.join(args.ckpt_path, 'log_custom.txt'))
        policy_gradient_train(policy_model, problems, train_pids, cand_pids, cand_examples, args, i)
